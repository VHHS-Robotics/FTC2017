...
@Autonomous(name = "Robot2_Red_Spot2_Encoder", group = "Linear Opmode")
public class Robot2_Red_Spot2_Encoder extends LinearOpMode {
...
//        SmallRelicServo = hardwareMap.servo.get("servo2");
        ...
        while (opModeIsActive() && (runtime.seconds() < 30.0)) {
            ...
            if (runtime.seconds() > 3.0 && runtime.seconds() < 7.5) {
                telemetry.addLine("I am turning");
                telemetry.update();

                double targetHeading = 275.00;
            ...
            if (runtime.seconds() > 7.5 && runtime.seconds() < 8.5) {
                stopRobot();
            }
            if (runtime.seconds() > 8.5  && runtime.seconds() < 13.5) { //All of them off by +3 in?
                if(glyphLocation==2) {    //RIGHT
                    encoderDrive(0.4, 25, 25, 12.5); // RIGHT
                    sleep(1500);
                }
                else if (glyphLocation==1) { //CENTER
                    encoderDrive(0.4, 33, 33, 12.75); // CENTER
                    sleep(1250);
                }
            ...
            if (runtime.seconds() > 13.5 && runtime.seconds() < 18.0) {

                double targetHeading = 185.00;
            ...
            if (runtime.seconds() > 22.5  && runtime.seconds() < 23.0) { // following the white line
            ...
            if (runtime.seconds() > 24.0  && runtime.seconds() < 24.5) { // following the white line
                goBack(.3);
            }
            if (runtime.seconds() > 24.5  && runtime.seconds() < 30.00) { // following the white line
                stopRobot();
            }
